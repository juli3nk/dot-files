#!/usr/bin/env bash

# https://github.com/boredomwontgetus/pa-vol

# get default sink name
SINK_NAME=$(pacmd dump 2> /dev/null | perl -a -n -e 'print $F[1] if /set-default-sink/')
if [ -z "$SINK_NAME" ]; then
        SINK_NAME="@DEFAULT_SINK@"
fi
# try this line instead of the one above if you got problems with the detection of your default sink.
#SINK_NAME=$(pactl stat | grep "alsa_output" | perl -a -n -e 'print $F[2]')

SOURCE_NAME=$(pacmd dump 2> /dev/null | perl -a -n -e 'print $F[1] if /set-default-source/')
if [ -z "$SOURCE_NAME" ]; then
        SOURCE_NAME="@DEFAULT_SOURCE@"
fi

# set max allowed volume; 0x10000 = 100%
VOL_MAX="0x10000"

STEPS="16" # 2^n
VOL_STEP=$((VOL_MAX / STEPS))

VOL_NOW=$(pactl get-sink-volume $SINK_NAME | awk -F '/' '/Volume/ { print $1 }' | sed 's/Volume: front-left: //')
MUTE_STATE=$(pactl get-sink-mute $SINK_NAME | awk -F ': ' '/Mute/ { print $2 }')
MIC_MUTE_STATE=$(pactl get-source-mute $SOURCE_NAME | awk -F ': ' '/Mute/ { print $2 }')

getvol() {
        BAR=""
        if [ $MUTE_STATE = yes ]; then
                BAR="mute"
                ITERATOR=$((STEPS / 2 - 2))
                while [ $ITERATOR -gt 0 ]; do
                        BAR=" ${BAR} "
                        ITERATOR=$((ITERATOR - 1))
                done
        else
                DENOMINATOR=$((VOL_MAX / STEPS))
                LINES=$((VOL_NOW / DENOMINATOR))
                DOTS=$((STEPS - LINES))
                while [ $LINES -gt 0 ]; do
                        BAR="${BAR}|"
                        LINES=$((LINES - 1))
                done
                while [ $DOTS -gt 0 ]; do
                        BAR="${BAR}."
                        DOTS=$((DOTS - 1))
                done
        fi
        echo "$BAR"
}

setvol() {
        pactl set-sink-volume $SINK_NAME ${1}%
}

plus() {
        VOL_NEW=$((VOL_NOW + VOL_STEP))
        if [ $VOL_NEW -gt $((VOL_MAX)) ]; then
                VOL_NEW=$((VOL_MAX))
        fi
        pactl set-sink-volume $SINK_NAME `printf "0x%X" $VOL_NEW`
}

minus() {
        VOL_NEW=$((VOL_NOW - VOL_STEP))
        if [ $(($VOL_NEW)) -lt $((0x00000)) ]; then
                VOL_NEW=$((0x00000))
        fi
        pactl set-sink-volume $SINK_NAME `printf "0x%X" $VOL_NEW`
}

mute() {
        pactl set-sink-mute $SINK_NAME toggle
}

micmute() {
        pactl set-source-mute $SOURCE_NAME 0
}

micunmute() {
        pactl set-source-mute $SOURCE_NAME 1
}

mictoggle() {
        pactl set-source-mute $SOURCE_NAME toggle
}

case "$1" in
        get)
                getvol
        ;;
        set)
                setvol $2
        ;;
        plus)
                plus
        ;;
        minus)
                minus
        ;;
        mute)
                mute
        ;;
        micmute)
                micmute
        ;;
        micunmute)
                micunmute
        ;;
        mictoggle)
                mictoggle
esac
